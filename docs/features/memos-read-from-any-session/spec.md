...
- **출력**(한 메모 한 항목, 개행 포함 content 를 버틴다 — 실측: jinwooauto 23건 중 2건이 다중 줄):
  ...
  - [ ] 2026-09-10 첫 줄 내용
      둘째 줄부터 4칸 들여쓰기로 verbatim 그대로
  - [x] 2026-09-09 완료된 메모
  ```

  헤더의 `완료 A · 미완료 B` 는 **필터 이전** 전체 카운트이고, 필터가 걸려 있으면 ` · 필터: done|undone`
  를 헤더에 붙인다 — 잘린 목록을 전체처럼 보이게 그리는 거짓 렌더를 막는다. 값은 캡틴이 친 플래그
  토큰을 그대로 쓴다(`--undone` → `undone`) — 입력과 출력이 같은 단어로 맞물려야 읽는 쪽(사람이든
  AI 든)이 "무엇을 걸렀나"를 다시 계산하지 않는다.
