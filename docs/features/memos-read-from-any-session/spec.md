...
`gootte memo [project-slug] [--done|--undone]` 를 신설한다. 그 프로젝트의 메모를 **내용 그대로**
목록으로 낸다. 기본 범위는 **실행한 cwd 의 프로젝트** — 인자를 주지 않는다.
...
- **범위**: 현재 프로젝트 하나. "전체 프로젝트" 모드는 **없다**(캡틴 답).
- **프로젝트 유추**: `resolveProjectArg`/`slugFromCwd` 재사용(`cli/src/commands.ts:69-86` — 이미
  "인자 생략 시 cwd 조상에서 발견 표식으로 찾는다" 는 관례가 있다). 새로 만들지 않는다.
...
- **고아 slug**: 발견 목록에 없어도 `memos/x.json` 이 있으면 그대로 출력한다(파일 존재가 사실).
