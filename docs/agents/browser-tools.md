...
**5. dev 서버는 내가 띄우고 내가 내린다.**
검증에 서버가 필요하면 착수 전에 포트가 비었는지 보고, 끝나면 **자기가 띄운 것만** 프로세스
트리를 따라 내린다. `pkill`·`killall` 같은 패턴 종료는 옆 사본과 캡틴 서버까지 죽인다
(루트 `AGENTS.md` §실행 명령). 캡틴 앱이 이미 떠 있으면 **다른 포트 + 다른 `GOOTTE_DATA_DIR`**
로 따로 띄워라 — 같은 데이터 디렉토리를 쓰면 캡틴의 계획 DB 를 오염시킨다.
