...
| **2e · roadmap-doc-browser** ✅ done | W — 웹 대시보드 | 이니셔티브 인라인 = Unix 디렉토리형 문서 브라우저(cd/ls) — 폴더 파일 + 가상 todo/ → tree 나열 → DocDrawer 재사용 | `TreeNode`·`DocRef`·`TreeResponse` seam·CORE·CONTRACT | 2a |
| **2f · web-structure** ✅ done | W — 웹 대시보드 | plan 보드(칸반) → 저작 `docs/mermaid` 구조 뷰(track 그룹·리스트 동형 드로어 뷰어·다크모드/문법 안전) · 칸반 교체(web-viz 칸반 부분 supersede) | CORE `buildStructure`·core-io `readMermaidDocs`·CONTRACT `Structure*` | 2c |
