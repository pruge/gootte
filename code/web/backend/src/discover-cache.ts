 ...
 * 감시 뿌리를 파생하는 `firstmateHome` 에도 달려 있다. `firstmateHome` 만 바꿔도 discover
 * 목록(roots)은 그대로라 키가 안 바뀌어 stale 카운트를 내준다(실측: firstmateHome 해제 후
 * 좌측 숫자가 안 바뀜). 그래서 `keySalt`(보통 `readSettings().firstmateHome`)를 키에 같이 넣어,
 * 홈이 바뀌면 다른 캐시로 빌드되게 한다. `keySalt` 미전달은 빈 문자열(기존 동작 보존).
 */
