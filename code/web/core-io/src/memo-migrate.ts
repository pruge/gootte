 ...
 *
 * 읽기는 아직 central 이다 — 쓰는 자리를 만들고(T01) → 이관하고 → 읽기를 돌린다(T02). 순서가
 * 강제가 되는 근거는 `grill.md` Locked 5·6(읽기를 먼저 돌리면 미이관 프로젝트의 메모가 화면에서
 * 사라진 것처럼 보인다). 그래서 이 파일은 **어디서 읽고 어디에 쓰는지를 모른다** — 주어진 두
 * 파일의 존재와 내용만 비교한다.
 *
