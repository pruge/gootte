...
/**
 * `plan` 탭(티켓 03) 응답 — 화면이 전체 개발 순서를 그리는 데 필요한 것을 한 번에 싣는다.
 ...
 *
 * `dragWarnings` 는 **지금 놓여 있는 자리 그대로**에 대해 04 의 같은 검사(`checkTicketDragWarnings`)를
 * 매 읽기 다시 돌린 것(INV-1·INV-4) — 키는 `${feature}/${ticket}`, 검사에 걸리는 티켓만 담긴다(빈 배열 생략).
 * 화면(티켓 09 ②)은 방금 끈 티켓의 키로 이 표를 찾아 보여줄 뿐, 스스로 판정하지 않는다 —
 * 배치가 바뀌면 다시 읽을 때 답도 같이 바뀐다("다시 물어서 갱신한다").
 * 🔴 `askTriggers`·`askRequests`(판단 요청, 티켓 06)는 09 가 걷어냈다 — 다시 만들지 않는다.
 */
export const PlanResponse = z.object({
  ...
  order: PlanOrder,
  next: NextResult,
  dragWarnings: z.record(z.array(DragWarning)).default({}),
});
