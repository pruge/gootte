...
export function DropList({ drops }: { drops: DropRecord[] }) {
  ...
  return (
    <section aria-labelledby="drop-heading">
      <h2 id="drop-heading" className="mono mb-2 text-sm tracking-[0.2em] text-muted">
        ── drop ({drops.length}) ──
      </h2>
      <ul className="space-y-1.5">
        {drops.map((d, i) => (
          <li key={`${d.todo}-${i}`} className="border-l-2 border-drop/40 py-1 pl-3 text-base">
            ...
            {d.initiative && (
              <p className="mono pl-1 text-sm text-muted opacity-70">↳ {d.initiative}</p>
            )}
          </li>
        ))}
      </ul>
