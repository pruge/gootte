 ...
 *
 * 🔴 판정 자리는 선행 계산에 이미 있는 값 둘뿐이다(INV-4) — `ticket.status`(레코드·문서
 * 확정 뒤의 최종 상태)와 `ticket.startable`(features.ts `toTicket` 이 "선행이 모두 풀렸고
 * 임자가 없다" 로 세팅한 값, finalize-status 가 신관례를 재판정). 여기서 새로 추정하지 않는다.
 *
