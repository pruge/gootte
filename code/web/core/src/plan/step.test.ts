...
describe("computeDisplaySteps — 당김은 표시 계산이다(INV-B2, plan-board/05)", () => {
  it("빈 단계 하나 — 기능별: a 는 완료돼 빠지고, b·c 는 각자 남은 티켓 하나씩이니 모두 1단계", () => {
  ...
  it("🔴 한 단계에 티켓이 하나라도 안 끝났으면 당기지 않는다", () => {
    ...
    // 다만 a-01(완료)은 표시 단계를 받지 않아 화면에서 걷히고, a 는 안 끝난 02 만 남는다.
    // b 도 남은 티켓이 하나뿐이니 기능별로는 1단계다(전역 단계 모델 아님).
  ...
  it("🔴 폐기와 미완이 같은 단계에 섞이면 안 빈다 — 미완 하나가 그 단계를 붙든다", () => {
    ...
    // 다만 a-01(폐기)은 표시 단계를 받지 않아 화면에서 걷히고, a 는 안 끝난 02 만 남는다.
    // b 도 남은 티켓 하나뿐이니 기능별로는 1단계다.
    const steps = [s("a", "01", 1), s("a", "02", 1), s("b", "03", 2)];
    expect(computeDisplaySteps(features, placements, steps)).toEqual({
      a: { "02-x": 1 },
      b: { "03-x": 1 },
...
describe("placeStep — 놓은 자리 → 저장 숫자(plan-board/08, 기능별)", () => {
  it("이미 있는 단계 위 — 그 단계가 원래 쓰던 저장 숫자를 그대로 돌려준다(기능별 스코프)", () => {
    // a 는 티켓 하나뿐(stored 1), b 는 다른 기능. a 안에서 onStep displayStep=2 는
    // clamp 돼 a 의 1단계(stored 1) 를 돌려준다 — 전역이었으면 b 의 stored 2 를 줬겠지만
    // 기능별라 a 안에서만 센다.
    const features = [feature("a", ["01"]), feature("b", ["02"])];
    const placements = [row("a", "active", 0), row("b", "active", 1)];
    const steps = [s("a", "01", 1), s("b", "02", 2)];
    expect(placeStep(features, placements, steps, { kind: "onStep", displayStep: 2 }, "a")).toBe(1);
  });

  it("같은 기능 안에서 onStep — N번째 저장 숫자를 돌려준다", () => {
    const features = [feature("a", ["01", "02", "03"])];
    const placements = [row("a", "active", 0)];
    const steps = [s("a", "01", 1), s("a", "02", 2), s("a", "03", 3)];
    expect(placeStep(features, placements, steps, { kind: "onStep", displayStep: 2 }, "a")).toBe(2);
  });
  ...
  it("단계와 단계 사이 — 앞뒤 저장 숫자의 중간값", () => {
    ...
    const placements = [row("a", "active", 0)];
    const steps = [s("a", "01", 1), s("a", "02", 2), s("a", "03", 3)];
    expect(placeStep(features, placements, steps, { kind: "gap", index: 1 }, "a")).toBe(1.5);
  });

  it("맨 앞 — 가장 작은 저장 숫자 − 1(걷힌 단계도 포함)", () => {
    const features = [feature("a", [resolved("01", "2026-08-01"), "02", "03"])];
    const placements = [row("a", "active", 0)];
    // a-01(완료, stored 1)은 화면에선 걷히지만 저장 숫자 1은 살아 있어 맨 앞은 1−1=0.
    const steps = [s("a", "01", 1), s("a", "02", 2), s("a", "03", 3)];
    expect(placeStep(features, placements, steps, { kind: "gap", index: 0 }, "a")).toBe(0);
  });

  it("번호 매겨진 단계들 맨 뒤 — 가장 큰 저장 숫자 + 1(걷힌 단계도 포함)", () => {
    const features = [feature("a", [resolved("01", "2026-08-01"), resolved("02", "2026-08-01"), "03"])];
    const placements = [row("a", "active", 0)];
    // a-01·a-02 는 완료(stored 1·2)지만 a-03 만 남아 stored 5 — 맨 뒤는 5+1=6.
    const steps = [s("a", "01", 1), s("a", "02", 2), s("a", "03", 5)];
    expect(placeStep(features, placements, steps, { kind: "gap", index: 1 }, "a")).toBe(6);
  });
  ...
  it("번호 매겨진 단계가 하나도 없을 때 맨 뒤 — 1", () => {
    ...
    expect(placeStep(features, placements, steps, { kind: "gap", index: 0 }, "a")).toBe(1);
  });
  ...
  it("9999 무더기 위 — 늘 9999", () => {
    ...
    expect(placeStep(features, placements, steps, { kind: "unranked" }, "a")).toBe(UNRANKED_STEP);
  });

  it("🔴 전부 완료돼 화면에서 걷힌 단계가 중간에 있어도 맨 뒤 숫자가 그 숫자와 안 부딪친다", () => {
    // a-01 은 완료(stored 1)돼 화면에서 걷혔지만 저장 숫자 1은 여전히 살아 있다.
    // a 의 남은 티켓은 02(stored 2)·03(stored 3) — 맨 뒤는 3+1=4. 걷힌 1 이 안 부딪친다.
    const features = [feature("a", [resolved("01", "2026-08-01"), "02", "03"])];
    const placements = [row("a", "active", 0)];
    const steps = [s("a", "01", 1), s("a", "02", 2), s("a", "03", 3)];
    expect(placeStep(features, placements, steps, { kind: "gap", index: 2 }, "a")).toBe(4);
    expect(placeStep(features, placements, steps, { kind: "gap", index: 0 }, "a")).toBe(0);
  });
});
