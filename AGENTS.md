...
| `gootte start [--at <TIME>] [--update] <기능> <티켓>` / `gootte cancel <기능> <티켓>` / `gootte pause [--at <TIME>] <기능> <티켓>` / `gootte resume [--at <TIME>] <기능> <티켓>` / `gootte end [--at <TIME>] <기능> <티켓>` / `gootte drop [--at <TIME>] <기능> [<티켓>]` | 티켓 문서에 `Time:` 줄을 기록한다. `start`=시작(이미 시작된 티켓이면 `started=` 갱신을 물어본다 — `--update` 로 묻지 않고 바로 갱신), `cancel`=시작 취소(Time 줄 삭제 — `finished=`/`paused=` 있으면 거절), `pause`=일시중단(paused=`<ISO>`), `resume`=재개(resumed=`<ISO>`), `end`=완료(finished=`<ISO>`). `end` 는 **구관례(`issues/`)만** `**Status:**` 줄을 `resolved (완료일)` 로 함께 갱신한다. 신관례(`tickets/`)는 done 을 `Time:` 줄에서 파생한다. `drop` 은 티켓을 폐기(wontfix)한다. `--at <TIME>` 로 시각 지정(비면 지금, ISO8601 또는 상대시간 `1h30m`/`90m`/`2h`/`1d`). `bin/gootte` — TS 모노레포(`@gootte/*`)를 전혀 참조하지 않는 독립 bash 스크립트, `npm i -g .`/`npm link` 로 어느 프로젝트에서든 전역 설치 가능. cwd 기준(git 루트 우선) — `<프로젝트>` 인자 없음. 커밋 안 함(파일만 편집). 여러 사본 중 기록 대상은 ADR-0002(자동 선택 우선, 모호하면 dialog). | 누구나 |
