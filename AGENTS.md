...
| `gootte start [--at <TIME>] <기능> <티켓>` / `gootte end [--at <TIME>] <기능> <티켓>` | 티켓 문서에 `Time: started=<ISO>`/`finished=<ISO>` 를 기록한다. `end` 는 **구관례(`issues/`)만** `**Status:**` 줄을 `resolved (완료일)` 로 함께 갱신한다(ready-for-agent 등이 방치되는 것 방지; 이미 resolved 면 건너뜀) — 신관례(`tickets/`)는 done 을 `Time:` 줄에서 파생하므로 Status 를 쓰지 않는다(drop 만 쓴다). 두 관례를 본다 — 신관례 `docs/features/<기능>/tickets/T<NN>.md`(`Time:` 줄)·구관례 `docs/features/<기능>/issues/<NN>-*.md`(`**Time:**` 줄). `--at <TIME>` 로 시각 지정(비면 지금, ISO8601 또는 상대시간 `1h30m`/`90m`/`2h`/`1d`). `bin/gootte` — TS 모노레포(`@gootte/*`)를 전혀 참조하지 않는 독립 bash 스크립트, `npm i -g .`/`npm link` 로 어느 프로젝트에서든 전역 설치 가능. cwd 기준(git 루트 우선) — `<프로젝트>` 인자 없음. 커밋 안 함(파일만 편집) | 누구나 |
